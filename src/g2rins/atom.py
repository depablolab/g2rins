...
as parsed from a G2RINS string. It includes handling for regular atoms,
...
class Atom(G2rinsBase, GenerationBase):
    """
    ...
    def _generate_partial_graph(self) -> _PartialGraph:
        ...
        Returns:
            _PartialGraph: A partial graph with the atom as a node
                                     and initialized left/right half-bonds.
        """
        g: nx.MultiDiGraph = nx.MultiDiGraph()
        node_id: str = str(uuid.uuid4())
        g.add_node(node_id, smi_text=str(self), obj=self)
        partial_graph: _PartialGraph = _PartialGraph(g)
